...
export default function NewProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    manufacturer: '',
    description: '',
    targetSites: [{ name: '', url: '', isActive: true }] as TargetSiteInput[],
    alertThreshold: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || '제품 생성에 실패했습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">새 제품 추가</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={handleInputChange}
                placeholder="예: STMicroelectronics"
                required
              />
            </div>
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          <Button type="submit" disabled={loading}>
            {loading ? <Loading size="sm" /> : <Save className="h-4 w-4 mr-2" />}
            저장
          </Button>
        </div>
      </form>
